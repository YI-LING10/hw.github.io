        ...
        <div class="col-4" id="content">
          <div style="width: 200px; height:500px; border-right: solid 1px gray;">
            ...
            <div class="vertical-menu">
              ...
              <a href="order.jsp"  class="active">訂單查詢</a>
...
<% 
                    //顯示訂單
					sql = "SELECT * FROM `product`,`orders`,`customers` WHERE (`product`.`ProductID`=`orders`.`ProductID`) AND (`orders`.`CustomersID`=`customers`.`CustomersID`) AND(`CustomersAccount`='" +session.getAttribute("id")+"')";
                    rs =  con.createStatement().executeQuery(sql);    
					while(rs.next()){
%> 
              <tr>
                <td><%=rs.getString("OrdersID")%></td>
                <td><%=rs.getString("PurchaseDate")%></td>	
                <td><%=rs.getString("ProductName")%></td>
                <td><%=rs.getString("PurchaseQuantity")%></td>
                
<%
				Integer price=Integer.valueOf(rs.getString("Price"));
				Integer quantity=Integer.valueOf(rs.getString("PurchaseQuantity"));
%>
        ...
        <div class="modal-content">
          <div class="modal-body" >
            <div class="tab">
			  ...
			  <button type="button" class="close mr-3 mt-2" data-dismiss="modal" style="font-size:30px;">&times;</button>
			  <%
			  }else{
			  %>
